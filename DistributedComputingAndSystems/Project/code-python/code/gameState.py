...
test = True
testic = { # relative positions for each player
    "Filip" : {"a":40,"b":42,"c":43,"d":39},
    "Dora" : {"a":40,"b":43,"c":41,"d":35},
    "Hari" : {"a":42,"b":37,"c":38,"d":40},
    }

class GameState():
    def __init__(self) -> None:
        self.dic_TEST_ONLY = None
    ...
    def play(self, player, forTestONLY = None): #return to none
        die = random.randint(1,6) if self.dic_TEST_ONLY == None else self.dic_TEST_ONLY
        ...
        if len(pieces) == 0:
            ...
            move_piece = self.getMovePiece(pieces) if forTestONLY == None else forTestONLY
